...
$(function() {
    ...
    $("#geocode-auto").click(function() {
        ...
        $("#manu-create-map").attr("disabled", "disabled");
        createMap();
    });
    $("#geocode-manu").click(function() {
        $("#lon").removeAttr("disabled");
        $("#lat").removeAttr("disabled");
        $("#manu-create-map").removeAttr("disabled");
        if ($("#lon").html() === "") {
            var geocodeHTML = "";
            $.each(gfields, function(index, field) {
                if (field.id != "_id" && (field.type === "number" || field.type === "int4" || field.type === "numeric")) {
                    geocodeHTML = geocodeHTML + "<option value='" + field.id + "'>" + field.id + "</option>";
                }
            });
            $("#lon").html(geocodeHTML);
            $("#lat").html(geocodeHTML);
        }
    });
    ...
    if (!iframeModel) {
        ...
        var packageId = $("#content>.toolbar>.breadcrumb>li:eq(3)>a:eq(0)", window.parent.document).attr("href").substring(9);
        $.ajax({
            type:"POST",
...
function getFilterSQL(NeedLimit) {
    ...
    sql = sql.replace(new RegExp(",$"), "") + ' FROM "' + resourceId + '" WHERE 1=1';
        ...
        if (filter.type === "range") {
            if (filter.from != "") {
                if (getFieldType(filter.id) === "numeric") {
                    ...
                    sql = sql + ' AND "' + filter.id + "\" != '' AND to_number(\"" + filter.id + "\", '999999999999D9999') >= " + filter.from;
                }
            }
            if (filter.to != "") {
                if (getFieldType(filter.id) === "numeric") {
                    ...
                    sql = sql + ' AND "' + filter.id + "\" != '' AND to_number(\"" + filter.id + "\", '999999999999D9999') <= " + filter.to;
                }
            }
        } else if (filter.type === "daterange") {
            if (filter.from != "") {
                sql = sql + ' AND to_date("' + filter.id + "\",'" + filter.format + "') >= to_date('" + filter.from + "','mm/dd/yyyy')";
            }
            if (filter.to != "") {
                sql = sql + ' AND to_date("' + filter.id + "\",'" + filter.format + "') <= to_date('" + filter.to + "','mm/dd/yyyy')";
            }
        } else if (filter.type === "dateterm") {
            sql = sql + ' AND to_date("' + filter.id + "\",'" + filter.format + "') = to_date('" + filter.value + "','mm/dd/yyyy')";
        } else if (filter.type === "timestamprange") {
            if (filter.from != "") {
                sql = sql + ' AND "' + filter.id + "\" >= to_date('" + filter.from + "','mm/dd/yyyy')";
            }
            if (filter.to != "") {
                sql = sql + ' AND "' + filter.id + "\" <= to_date('" + filter.to + "','mm/dd/yyyy')";
            }
        } else if (filter.type === "timestampterm") {
            sql = sql + ' AND "' + filter.id + "\" = to_date('" + filter.value + "','mm/dd/yyyy')";
        } else if (filter.type === "term") {
            if (getFieldType(filter.id) === "numeric") {
                sql = sql + ' AND "' + filter.id + '" = ' + filter.to;
            } else {
                sql = sql + ' and "' + filter.id + "\"=E'" + valReplace(filter.value) + "'";
            }
        } else if (filter.type === "contain") {
            sql = sql + ' and "' + filter.id + "\" ILIKE E'%!" + valReplace(filter.value) + "%'ESCAPE '!'";
        }
    });
    if (currentSortCol != null) {
...
function addAnFilter() {
    ...
    if (filterField != "" && filterField != null) {
        ...
        switch (filterType) {
          case "term":
            filterHTML = "<fieldset forField='" + filterField + "' forType='term'><legend style='font-size: 14px; margin-bottom: 2px;'> " + filterField + " <small>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</small><a class='removeFilter'>×</a></legend><label>Equal</label><input name='term' type='text'></fieldset>";
            break;

          case "contain":
            filterHTML = "<fieldset forField='" + filterField + "' forType='contain'><legend style='font-size: 14px; margin-bottom: 2px;'> " + filterField + " <small>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</small><a class='removeFilter'>×</a></legend><label>Contain</label><input name='contain' type='text'></fieldset>";
            break;

          case "range":
            filterHTML = "<fieldset forField='" + filterField + "' forType='range'><legend style='font-size: 14px; margin-bottom: 2px;'> " + filterField + " <small>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</small><a class='removeFilter'>×</a></legend><label>From</label><input name='range-from' type='text'><label>To</label><input name='range-to' type='text'></fieldset>";
            break;

          case "dateterm":
            filterHTML = "<fieldset forField='" + filterField + "' forType='dateterm' dateformat='" + $("#filter-field-type").attr("format") + "'><legend style='font-size: 14px; margin-bottom: 2px;'> " + filterField + " <small>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</small><a class='removeFilter'>×</a></legend><label>Equal</label><input name='date-term' type='text'></fieldset>";
            break;

          case "daterange":
            filterHTML = "<fieldset forField='" + filterField + "' forType='daterange' dateformat='" + $("#filter-field-type").attr("format") + "'><legend style='font-size: 14px; margin-bottom: 2px;'> " + filterField + " <small>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</small><a class='removeFilter'>×</a></legend><label>From</label><input name='range-from' type='text'><label>To</label><input name='range-to' type='text'></fieldset>";
            break;

          case "timestampterm":
            filterHTML = "<fieldset forField='" + filterField + "' forType='timestampterm'><legend style='font-size: 14px; margin-bottom: 2px;'> " + filterField + " <small>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</small><a class='removeFilter'>×</a></legend><label>Equal</label><input name='date-term' type='text'></fieldset>";
            break;

          case "timestamprange":
            filterHTML = "<fieldset forField='" + filterField + "' forType='timestamprange'><legend style='font-size: 14px; margin-bottom: 2px;'> " + filterField + " <small>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</small><a class='removeFilter'>×</a></legend><label>From</label><input name='range-from' type='text'><label>To</label><input name='range-to' type='text'></fieldset>";
            break;
        }
        if ($("#filters").html() === "") {
        ...
        if (filterType === "daterange" || filterType === "timestamprange") {
            ...
            $(lastRangeFrom).datepicker({
                changeMonth:true,
                changeYear:true
            });
            $(lastRangeTo).datepicker({
                changeMonth:true,
                changeYear:true
            });
        }
...
function addSeries() {
    ...
    if (groupColumnType === "value") {
        seriesHTML = "<fieldset><label><span class='series-span'>Series " + words[seriesNum - 1] + "(Y-Axis)</span>[<a>Remove</a>]</label><select name='series-type' class='series-type' style='width:80px;margin-right:2px;'><option value='value'>Value</option></select><select name='series-field' class='series-field' style='width:108px;'>";
    } else {
        seriesHTML = "<fieldset><label><span class='series-span'>Series " + words[seriesNum - 1] + "(Y-Axis)</span>[<a>Remove</a>]</label><select name='series-type' class='series-type' style='width:80px;margin-right:2px;'><option value='count'>Count</option><option value='sum'>Sum</option><option value='max'>Max</option><option value='min'>Min</option><option value='avg'>Avg</option></select><select name='series-field' class='series-field' style='width:108px;'>";
    }
    var seriesY = new Array();
    var seriesFieldSets = $("#series").find("fieldset");
    $.each(gfields, function(index, field) {
        if (field.id != "_full_count" && field.id != "rank") {
            if (groupColumnType != "value" || field.type === "number" || field.type === "int4" || field.type === "numeric") {
                seriesY.push(field);
            }
        }
    });
    ...
    $.each(seriesFieldSets, function(index, sfs) {
        ...
        if (index === 0) {
            ...
            if (seriesVal === "" && columnType != "groupby") {
                ...
                seriesHTML = "<option value='" + seriesVal + "' selected='selected'>" + seriesVal + "</option>" + (columnType != "groupby" ? "<option value=''>No column selected</option>" :"");
            }
            ...
            if (seriesType === "count" || fieldType === "number" || fieldType === "int4" || fieldType === "numeric") {
                seriesHTML = "<option value='" + seriesVal + "' selected='selected'>" + seriesVal + "</option>";
            }
            $.each(seriesY, function(i, y) {
                if (y.id != "_id" && (seriesType === "count" || y.type === "number" || y.type === "int4" || y.type === "numeric")) {
                    seriesHTML = seriesHTML + "<option value='" + y.id + "'>" + y.id + "</option>";
                }
            });
        }
        ...
        if (groupColumn.val() != "_id" && groupColumn.find("option").length === 2 || groupColumn.find("option").length === 1) {
...
function reDrawGraph() {
    ...
    if (grecords.length > 0 && advancedGraph) {
        ...
        $.post(el.endPoint + "api/3/action/datastore_search_sql?sql=" + sql, function(data) {
...
function createEmbedCode() {
    ...
    var iframeURL = window.location.href + "?iframe=true&charttype=" + chartType + "&offset=" + offset + "&limit=" + limit;
    if (currentSortCol != null) {
        iframeURL = iframeURL + "&currentsortcol=" + window.encodeURI(JSON.stringify(currentSortCol)) + "&isasc=" + isAsc;
    }
    if (lastQuery != undefined) {
        if (lastQuery === "search") {
            ...
            if (filtersArr.length > 0) {
                ...
                iframeURL = iframeURL + "&filters=" + window.encodeURI(JSON.stringify(filtersArr)) + "&gfields=" + window.encodeURI(JSON.stringify(gfields));
            }
        }
    }
    if (chartType != "pie" && chartType != "pie3d") {
        ...
        iframeURL = iframeURL + "&groupcolumntype=" + $("#group-column-type").val() + "&groupcolumn=" + $("#group-column").val() + "&fields=" + window.encodeURI(JSON.stringify(syList));
    } else {
...
function processData(data) {
    ...
    if (data.result.total != undefined) {
        ...
        if (data.result.sql === undefined) {
            ...
            $.post(el.endPoint + "api/3/action/datastore_search_sql?sql=SELECT COUNT(*) AS total_count FROM (" + getFilterSQL(false) + ") AS filter_table", function(data) {
                $("#doc-count").html(data.result.records[0]["total_count"]);
            }).done();
        }
    }
    if (records != null) {
...
function createMap() {
    ...
    var addressField;
    var geocode = false, latField, lonField, latType, lonType;
    initMap = false;
    if ($("#geocode-manu:checked").length > 0) {
        latField = $("#lat").val();
        lonField = $("#lon").val();
        hasAddress = false;
        geocode = true;
        ...
        $.each(gfields, function(index, field) {
            if (field.id.toLowerCase() === "lon") {
                lonField = field.id;
                lonType = getFieldType(field.id);
            }
            if (field.id.toLowerCase() === "lat") {
                latField = field.id;
                latType = getFieldType(field.id);
            }
            if (latType != undefined && lonType != undefined && (latType === "number" || latType === "int4" || latType === "numeric") && (lonType === "number" || lonType === "int4" || lonType === "numeric")) {
                hasAddress = false;
                geocode = true;
                return false;
            }
    ...
    if (geocode) {
        $.each(grecords, function(index, record) {
            if (checkUSGeocode(record)) {
                featureGroup.addLayer(createMarker(record[latField], record[lonField], record));
            }
        });
        if (noBoundery) {
            fitMapBounds();
    ...
    function checkUSGeocode(record) {
        var lat = record[latField], lon = record[lonField];
    ...
    function createMarker(lat, lng, record) {
        ...
        marker.on("mouseover", function() {
            ...
            leafletPopup.find(".leaflet-popup-tip-container").css({
                "padding-bottom":"30px",
                position:"absolute",
                left:leftWidth + "px"
            });
...
function escapeRule(str) {
    return str.replace(/\+/g, "%2B").replace(/\?/g, "%3F").replace(/\//g, "%2F").replace(/\&/g, "%26").replace(/\#/g, "%23").replace(/\!/g, "%21").replace(/\|/g, "%7C").replace(/\=/g, "%3D").replace(/\^/g, "%5E").replace(/\`/g, "%60").replace(/\{/g, "%7B").replace(/\}/g, "%7D").replace(/\]/g, "%5D").replace(/\[/g, "%5B").replace(/\"/g, "%22").replace(/\</g, "%3C").replace(/\>/g, "%3E");
}